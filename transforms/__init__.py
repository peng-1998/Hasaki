...
class Compose(Module):
    r'The transform list like it in torchvision.transforms.Unlike torchvision.transforms.Compose,this Compose can get multiple input. \
...
class TransformPart(Module):
    r'Only transform i-th image in inputs for i in \'indexes\'. \
    For example: \
...
class TransformOnlyFrist(TransformPart):
    r'Only transform frist image in inputs. \
    For example: \
...
class TransformForAll(Module):
    r'Transform all images use \'transform\'.But don\'t use it with random transforms. \
    This class is use to make class in torchvision.transforms get multiple input.'
