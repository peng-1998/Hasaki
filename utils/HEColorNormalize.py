...
class HE_ColorNormalization(Module):
    ...
    def __init__(self, target_img: str | Tensor = None, use_cuda: bool = False, W_target: Tensor = None, r: int = 2, Lambda: float = 0.1) -> None:
        ...
        Args:
            target_img (str | Tensor, optional): The path of target image.When W_target is None, this parameter cannot be None. Defaults to None.
            use_cuda (bool, optional):Using CUDA acceleration, non-negative matrix factorization takes several seconds to iterate. Defaults to False.
            W_target (Tensor, optional): The target matrix W.When target_img is None, this parameter cannot be None. Defaults to None.
            r (int, optional): Matrix W,H will be W:(C,r) H:(r,N). Defaults to 2.
            Lambda (float, optional): Coefficients of sparse control items. Defaults to 0.1.
        """        
        super().__init__()
        ...
        if W_target:
            ...
            if isinstance(target_img, str):
                ...
                raise TypeError(f'"target_img" is expected to be the path of image file or an float image Tensor, but get type {type(target_img)}')
